...
from hermes_gen.ebnf_grammer import parse_grammer, Grammer
...
def main():
    ...
    parser.add_argument(
        '-t', '--token', default="token.h", help="Overrides the token header filename, defaults to 'token.h'"
    )
    parser.add_argument("-c", '--code', help="Override the code filename, defaults to [grammer_filename]_parser.cpp")
    ...
    folder, _ = os.path.split(codeFile)
    os.makedirs(folder, exist_ok=True)

    with open(codeFile, mode='w') as f:
        f.write(
            "/*******\n"
        ...
        f.write("using reductionFunc = HERMES_RETURN (*)(std::vector<HERMES_RETURN>);\n")

        for idx, rule in enumerate(grammer.rules):
            f.write(f"HERMES_RETURN r{idx}(std::vector<HERMES_RETURN> values)\n"
                    "{\n"
                    f"    {rule.code}\n"
                    "}\n")
        ...
        for idx, rule in enumerate(grammer.rules):
            f.write(f'r{idx}')
            if idx + 1 < len(grammer.rules):
                f.write(',')
            f.write("\n")
        f.write("};\n")  # End reduction func list
